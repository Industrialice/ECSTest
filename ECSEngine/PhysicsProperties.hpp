...
namespace ECSEngine
{
	struct PhysicsProperties
	{
		...
		optional<Vector3> inertiaTensor = nullopt; // if not set it will be computed based on attached colliders, 0 means infinity inertion
		optional<Quaternion> inertiaTensorRotation = nullopt; // if not set it will be computed based on attached colliders
		optional<ui32> solverPositionIterations = nullopt; // when not set the value from the physics settings will be used
